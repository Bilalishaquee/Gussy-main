  ...
  return (
    <div className="w-full py-6 sm:py-10">
      <div className="container px-4 mx-auto">
        <h1 className="mb-6 sm:mb-8 text-3xl font-bold text-center font-brygada">Featured Celebrities</h1>

        <div className="relative">
          {/* Left Arrow */}
          <button
            onClick={prevSlide}
            className="absolute z-10 w-8 h-8 sm:w-10 sm:h-10 border border-[#1B1C57] text-[#1B1C57] rounded-full flex items-center justify-center -left-4 sm:-left-6 top-1/2 transform -translate-y-1/2 hover:bg-gray-100 transition-all duration-200"
            aria-label="Previous"
          >
            <ChevronLeft size={20} />
          </button>
...
<div
  ...
  style={{
    width: `${shows.length * 25}%`, // 4 cards at a time = 25% each
...
>
  {shows.map((show) => (
    <div
      key={show.id}
      className="w-1/4 flex justify-center px-4"
    >
      <div className="rounded-lg overflow-hidden bg-gray-50 border shadow-sm w-[270px]">
        <img
          src={show.image}
          alt={show.title}
        ...
        <div className="px-3 py-3 font-afacad">
          ...
          <button
            onClick={nextSlide}
            className="absolute z-10 w-8 h-8 sm:w-10 sm:h-10 border border-[#1B1C57] text-[#1B1C57] rounded-full flex items-center justify-center -right-4 sm:-right-6 top-1/2 transform -translate-y-1/2 hover:bg-gray-100 transition-all duration-200"
            aria-label="Next"
          >
...
const MobileShowsListing: React.FC<{ shows: Show[] }> = ({ shows }) => {
  ...
  return (
    <div className="block sm:hidden px-4 pb-12 space-y-8">
      <h2 className="text-2xl font-bold text-center font-brygada">Find Celebrities</h2>
